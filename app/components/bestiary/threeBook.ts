...
export function createBook(
  ...
  for (const sy of [0.27, -0.27]) {
    ...
    // КОРЕНЬ ЛЕЖИТ ЧЕРЕЗ ВСЮ БЛЯШКУ. Ремень входит в ушко и идёт по лицу
    // накладки до её дальнего края (бляшка от −0.154 до +0.046 относительно
    // нуля группы) — как застёжной ремень поверх замка. STRAP_OVER — насколько
    // корень уходит за ноль к корешку; спуск и хвост не меняются.
    const STRAP_OVER = 0.16;
