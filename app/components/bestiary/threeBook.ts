...
export function createBook(
  ...
  for (const sy of [0.27, -0.27]) {
    ...
    // Корень спереди — как было: заканчивается на нуле группы, в прорези ушка
    // бляшки (край с прорезью у +0.046 после зеркала). Поверх лица накладки
    // ремень НЕ идёт — так лежит только хвост на ответной планке сзади.
    const strapW = 0.47 + STRAP_TAIL_EXTRA;
