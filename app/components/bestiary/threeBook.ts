...
export function createBook(
  ...
  for (const sy of [0.27, -0.27]) {
    ...
    // ХВОСТ ДОХОДИТ ДО ЗАМКА. Раньше длина давала хвосту по заднику всего ~0.013:
    // ремень едва касался края ответной бляшки и «не доставал до замка». Плюс
    // 0.17 — хвост ложится на лицо ответной бляшки (ширина 0.2) до её дальнего
    // края, остриём текстуры к корешку — как настоящий застёжной ремень.
    const STRAP_TAIL_EXTRA = 0.17;
    // Корень спереди заканчивается в прорези ушка бляшки (край с прорезью у
    // +0.046 после зеркала) и заходит за ноль группы к корешку на STRAP_OVER —
    // «чутка левее», чтобы конец с заклёпкой лёг на бляшку, а не висел у ушка.
    // Поверх всего лица накладки ремень НЕ идёт — так лежит только хвост на
    // ответной планке сзади. Дальний конец не двигается: длина растёт на OVER.
    const STRAP_OVER = 0.03;
    const strapW = 0.47 + STRAP_TAIL_EXTRA + STRAP_OVER;
