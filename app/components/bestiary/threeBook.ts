...
export function createBook(
  ...
  for (const sy of [0.27, -0.27]) {
    ...
    // ОСТРИЁ — В ГЕОМЕТРИИ, а не только в альфе. Лицо ремня режется альфой
    // текстуры в остриё (по замеру strap.webp сужение идёт с u≈0.86 к точке на
    // u=1), но коробка оставалась ПРЯМОУГОЛЬНОЙ до конца: боковые грани светлого
    // среза и торец торчали за остриём тонкими полосками — «полосы на кончиках».
    // Сужаем сами вершины по тому же профилю: срез сходится в точку вместе с
    // кожей, торец вырождается в ноль. Заодно тень ремня (customDepthMaterial
    // без альфы, см. ниже) перестаёт быть прямоугольной.
    {
      const TIP_FROM = 0.86;
      const spos = sgeo.attributes.position;
      for (let i = 0; i < spos.count; i++) {
        const t = spos.getX(i) / strapW; // 0..1 вдоль ремня: геометрия сдвинута от нуля
        if (t <= TIP_FROM) continue;
        const k = Math.max(0, 1 - (t - TIP_FROM) / (1 - TIP_FROM));
        spos.setY(i, spos.getY(i) * k);
      }
  ...
  function render(open: number, turn: number, leftPages: number, rightPages: number): void {
    ...
    camera.lookAt(lookX, closeUp ? 0.02 : narrow ? 0.06 : 0, 0);
  ...
  return {
    ...
    compile(): Promise<void> {
      ...
      //
      // Теневой проход ТОЖЕ греется — помешно, тем же шагом.
      //
      // Раньше его не грели осознанно: shadowMap.enabled входит в ключ
      // программы, и когда тени включались лишь по готовности, кадр с тенью под
      // заставкой собирал весь цветовой набор второй раз (замер: 3.3 → 6.7 с).
      // Теперь тени включены навсегда при сборке (см. shadowMap.enabled выше):
      // цветовые программы и так собраны с теневыми дефайнами, а depth-варианты
      // касателей — их считанные штуки — без прогрева линковались в момент
      // первого показа: 217 мс посреди анимации раскрытия и два провала по
      // ~200 мс на перелистывании (замер на проде по rAF главного потока).
      // needsUpdate перед каждым помешным кадром компилирует depth-программу
      // только что открытого меша, и пауза темпа срабатывает на неё так же,
      // как на цветовую. Листы в закрытой книге невидимы — им ниже отдельные
      // позы: раскрытая книга и лист на полпути в обе стороны.
      return renderer.compileAsync(scene, camera).then(async () => {
        ...
        // Позы, которых у плитки нет. Лист на 0.35 хода показывает тени лицевую
        // сторону, на 0.8 — изнанку (leafBack): у них разные depth-материалы.
        // Холст под заставкой, кадры никто не видит; последним рисуем плитку,
        // чтобы к готовности на холсте стояла закрытая книга, а не разворот.
        const warmPose = async (open: number, frac: number): Promise<void> => {
          renderer.shadowMap.needsUpdate = true;
          render(open, frac, page, LEAVES - page);
          const now = renderer.info.programs?.length ?? 0;
          if (now > known) {
            known = now;
            await new Promise<void>((res) => setTimeout(res, pace.ms));
        ...
        await warmPose(1, 0.35);
        await warmPose(1, 0.8);
        renderer.shadowMap.needsUpdate = true;
        render(cur.open, 0, page, LEAVES - page);
        moved = true;
      });
