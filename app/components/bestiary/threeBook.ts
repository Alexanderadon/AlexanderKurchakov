 ...
 */
// Прямой участок на крышке ДЛИННЕЕ, чем был (0.058): с торца ремень входил в
// бляшку по прямой, без читаемого загиба через кромку крышки. Радиус на углу
// тоже больше — это и есть загиб. Бляшки при этом уезжают от обреза на ~0.07.
const STRAP_FLAT = 0.12;
const STRAP_R = 0.04;
...
export function createBook(
  ...
  //
  // Решается ОДИН раз на всю жизнь сцены и больше не трогается.
  //
  // Была попытка включать теневой проход только в развороте — там он виден, а в
  // плитке 273×210 нет. Симптом вылез сразу: книга пропадала, страница вставала,
  // книга появлялась снова. Причина в том, что shadowMap.enabled входит в ключ
  // программы, а three освобождает программу, как только на неё не осталось
  // ссылок. Каждое переключение = удалить двадцать с лишним программ и собрать
  // столько же новых, и так в обе стороны: на открытии книги и на закрытии.
  //
  // Платим один раз, под заставкой, где прогрев идёт по нескольку мешей за кадр
  // и никого не держит. Дальше открытие и закрытие не стоят ничего.
  ...
  for (const sy of [0.27, -0.27]) {
    ...
    // Ушко с прорезью (три дырки) обязано смотреть К ОБРЕЗУ — туда, откуда
    // приходит ремень: корень с заклёпкой ложится в прорезь, и крепление
    // читается. Текстура нарисована ушком влево (к корешку) — зеркалим группу.
    // Отрицательный масштаб three отрабатывает: обход граней переворачивается,
    // а тангенс карты нормалей берётся из производных UV и зеркалится вместе с ней.
    plate.scale.x = -1;
    ...
    // Ремень входит в ушко бляшки (её край с прорезью — у x≈0.046 после
    // зеркала) и по ту сторону накладки выходит приклёпанным хвостом (см. ниже).
    // STRAP_OVER — насколько корень заходит за ноль группы; отрицательное
    // значение ставит корень у самого ушка. Спуск и хвост до ответной планки
    // не меняются: длина корректируется на ту же величину.
    const STRAP_OVER = -0.03;
    const strapW = 0.548 + STRAP_OVER;
    ...
    sgeo.translate(strapW / 2 - STRAP_OVER, 0, 0);
    ...
    // Срез кожи чуть светлее лица — сырая сердцевина ремня бледнее, и с ребра
    // ремень читается толщиной, а не плоской лентой. Но НЕ настолько, чтобы
    // кромка шла по ремню светлой обводкой: прежний 0x54432f вдвое ярче кожи
    // рисовал полосу вдоль всего ремня и на кончике. Теперь — на треть.
    edgeSkin.material.color.setHex(0x3b2f22);
    ...
    // ХВОСТ ПО ТУ СТОРОНУ БЛЯШКИ. Ремень уходит под ушко, проходит под накладкой
    // и выходит с корешковой стороны, где приклёпан к ней. Отдельный короткий
    // кусок той же кожи с корневым участком текстуры (закруглённый конец +
    // заклёпка). Живёт на группе бляшки, а не на шарнире: при расстёгивании
    // ремень отходит, а приклёпанный хвост остаётся на бляшке.
    {
      const TAIL_L = 0.085;
      const tgeo = new BoxGeometry(TAIL_L, strapH, 0.019, 6, 4, 1);
      tgeo.translate(-0.15 + TAIL_L / 2, 0, 0);
      const tpos = tgeo.attributes.position;
      for (let i = 0; i < tpos.count; i++) {
        if (tpos.getZ(i) < 0.0094) continue;
        const q = Math.cos((tpos.getY(i) / (strapH / 2)) * (Math.PI / 2));
        tpos.setZ(i, 0.0095 + 0.005 * q);
      }
      tgeo.computeVertexNormals();
      // Текстура — только корневые 13% ремня: закруглённый конец и заклёпка.
      const tuv = tgeo.attributes.uv;
      for (let i = 0; i < tuv.count; i++) tuv.setX(i, tuv.getX(i) * 0.13);
      tuv.needsUpdate = true;
      const tail = new Mesh(tgeo, [
        edgeSkin.material,
        edgeSkin.material,
        edgeSkin.material,
        edgeSkin.material,
  ...
  function render(open: number, turn: number, leftPages: number, rightPages: number): void {
    ...
    // Узкий раскрытый кадр — АНФАС. Ракурс, красивый на широком экране,
    // на портретном перекашивает разворот: вписывание считает прямоугольник в
    // лоб, а боковой поворот выносит дальний край за грань кадра — правая
    // страница резалась при честных размерах холста. К концу раскрытия узкий
    // кадр выпрямляется: рысканье в ноль, тангаж до лёгких восьми градусов.
    const basePitch = closeUp ? 7 : narrow ? 16 - 2 * anticip - 8 * swing : 20 - 2 * anticip + 7 * swing;
    const pitch = ((basePitch + overshoot) * Math.PI) / 180 + orb.pitch * orbW;
    const baseYaw = closeUp ? 0 : narrow ? -9 + 1 * anticip + 9 * swing : -16 + 2 * anticip + 4 * swing;
    const yaw = (baseYaw * Math.PI) / 180 + orb.yaw * orbW;
    // В узком кадре подходим заметно ближе и целимся в одну страницу.
    const designDist = closeUp
      ? 3.0
      : narrow
        ? 2.62 + 0.5 * swing + overshoot * 0.02
        // Закрытый том в модалке стоял впритык к холсту (2.78). Дальше на 0.3:
        // у книги появляется воздух; к раскрытию сумма та же 3.33 — рамка
        // разворота не меняется.
        : 3.08 + 0.25 * swing - 0.06 * anticip + overshoot * 0.02;
    // ВПИСЫВАНИЕ. Авторские дистанции выше подобраны под широкий кадр. На
    // портретном телефоне вертикальный угол камеры тот же, а горизонтальный
    // сжимается вместе с аспектом — и на прежней дистанции том вылезал за
    // холст: у закрытой книги срезало застёжки и углы, у разворота — правый
    // край и верх. Считаем дистанцию, с которой целевой прямоугольник
    // (закрытый том или одна страница с полями) гарантированно помещается по
    // обеим осям, и берём максимум с авторской. На широких экранах вписывание
    // всегда короче авторской дистанции и кадр НЕ меняется — это проверено
    // сверкой с эталонами; на узких оно берёт верх и книга видна целиком.
    const tanV = Math.tan((camera.fov * Math.PI) / 360);
    // Поля сняты с натуры: 1.46 у закрытого тома — чтобы в кадр попадали
    // КОНЧИКИ ремешков, обнимающих передний обрез (на 1.34 их срезало кромкой).
    // В узком раскрытом кадре — ВЕСЬ разворот (2.3), а не одна страница:
    // прежний прицел в правую страницу пользователь забраковал по существу —
    // левую половину книги на телефоне нельзя было увидеть вовсе, никаким
    // листанием. Целиком и мельче честнее, чем крупно и половиной.
    const fitW = closeUp ? 1.46 : 1.46 + (narrow ? 0.84 : 1.04) * swing;
    // 1.56, а не выше: при 1.66 вписывание по ВЫСОТЕ обгоняло авторскую
    // дистанцию уже на десктопе (2.90 против 2.78) и отодвигало закрытый том
    // в модалке на четыре процента — сверка с эталоном поймала. Том высотой
    // ~1.55 в 1.56 помещается впритык, а поле сверху и снизу добирает слагаемое
    // +0.18 к дистанции.
    const fitH = 1.56;
    const fitDist = Math.max(fitW / (2 * tanV * camera.aspect), fitH / (2 * tanV)) + 0.18;
    const dist = Math.max(designDist, fitDist);
    ...
    // В узком кадре смотрим чуть выше центра: пикирующий тангаж проецирует том
    // в верхнюю часть холста, и внизу оставалась пустая треть.
    // Закрытый том в модалке сидел ниже центра (по замеру на трёх окнах —
    // ровно на 0.10 единицы): цель взгляда на 0.05 ниже, и книга встаёт по
    // центру с равными зазорами. К раскрытию поправка исчезает.
    const lookY = closeUp ? 0.02 : narrow ? 0.06 : -0.05 * (1 - swing);
  ...
  return {
    ...
    setFraming(mode: "tile" | "spread"): void {
      ...
      glow.visible = !closeUp;
      shadowCatch.visible = !closeUp;
      // shadowMap.enabled здесь НЕ трогается: см. его установку при сборке.
    ...
    compile(): Promise<void> {
      ...
      // чтобы слинковать depth-программы заранее. Замер показал обратное —
      // первая загрузка подорожала с 3.3 до 6.7 с. Причина в том, что
      // shadowMap.enabled входит в ключ программы: кадр с тенью собирает ВЕСЬ
      // цветовой набор заново, с другими дефайнами, и возврат к плитке собирает
      // его в третий раз. Двадцать одна программа компилируется дважды ради
      ...
      return renderer.compileAsync(scene, camera).then(async () => {
        ...
        // По ОДНОМУ мешу за проход и с настоящей паузой между ними.
        //
        // GPU-процесс в браузере один на всё: на заставку, на курсор, на этот
        // прогрев — в каком бы потоке ни жила сцена. Когда прогрев жмёт драйвер
        // очередью встык (следующая программа сразу за предыдущей), на слабых
        // видеокартах встаёт композитор ЦЕЛИКОМ: на видео пользователя курсор
        // оставлял шлейф призрачных стрелок — экран не успевал перерисоваться.
        // Пауза в полсотни миллисекунд между касаниями программ даёт композитору
        // кадры на дыхание. Прогрев удлиняется на пару секунд, но идёт он под
        // заставкой в воркере, где его времени никто не считает; рывки системы —
        // считает пользователь.
        // Пауза — ТОЛЬКО когда касание реально отдало драйверу новую программу:
        // меши, чей материал уже тёплый, идут без остановки. Иначе платили бы
        // полсотни миллисекунд за каждый из полусотни мешей, и перезагрузка
        // растягивалась на пустых паузах (замер: оверлей 3.2 → 5.6 с).
        let known = renderer.info.programs?.length ?? 0;
        for (let i = 0; i < meshes.length; i += 1) {
          meshes[i].visible = wasVisible[i];
