...
export function createBook(
  ...
  for (const sy of [0.27, -0.27]) {
    ...
    // Корень ремня ЛЕЖИТ НА БЛЯШКЕ: ремень проходит через ушко и ложится на её
    // лицо до середины, где заклёпка, — как настоящий застёжной ремень. STRAP_OVER
    // — насколько корень уходит за ноль группы (к корешку), поверх бляшки. Длина
    // выросла на ту же величину: хвост до ответной планки и спуск не менялись.
    // Отрицательный x в шейдере обхода — прямой участок (min(s, FLAT) = s).
    const STRAP_OVER = 0.09;
