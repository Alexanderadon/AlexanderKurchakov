...
test.describe("прелоадер", () => {
  ...
  test("в консоли нет ошибок", async ({ page }) => {
    ...
    page.on("console", (m) => {
      // Медиа в тестах блокируется намеренно (см. e2e/base.ts — иначе каждый
      // прогон играет hero.mp4 в колонки). Браузер честно пишет об этом в
      // консоль, и к прелоадеру эта запись отношения не имеет.
      const t = m.text();
      // Медиа в тестах блокируется намеренно (см. e2e/base.ts — иначе каждый
      // прогон играет hero.mp4 в колонки), и браузер честно пишет об этом в
      // консоль БЕЗ адреса, одной строкой. Отличить её от настоящего сбоя
      // загрузки по тексту нельзя, поэтому отсекаем весь класс: этот тест про
      // ошибки исполнения в прелоадере, а доступность ассетов проверяется
      // отдельно — тем, что прелоадер вообще доходит до конца.
